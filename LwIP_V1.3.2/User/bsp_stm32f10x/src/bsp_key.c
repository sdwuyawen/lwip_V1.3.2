...
*		V1.1    2013-06-29 		   增加1个读指针，用于bsp_Idle() 函数读取系统控制组合键（截屏）
...
static void bsp_InitKeyVar(void)
{
	...
	s_tBtn[KID_K1].RepeatSpeed = 50;	/* 每隔500ms自动发送键值 */
	
	s_tBtn[KID_K2].LongTime = 100;
	s_tBtn[KID_K2].RepeatSpeed = 0;	/* 每隔100ms自动发送键值 */
...
//			printf("Fault : DetectButton(), s_tBtn[i].IsKeyDownFunc undefine");
	...
	if (pBtn->IsKeyDownFunc())						//当前按键是按下状态
	{
		...
		else										//前沿抖动已经滤除
		{
			...
			if (pBtn->LongTime > 0)
			{
				if (pBtn->LongCount < pBtn->LongTime)//这里一次按键只进入一次
				{
					/* 发送按钮持续按下的消息 */
	...
	{
		if(pBtn->Count > KEY_FILTER_TIME)	//滤除后沿抖动
		{
			pBtn->Count = KEY_FILTER_TIME;
		}
		else if(pBtn->Count != 0)			//滤除后沿抖动
		{
			pBtn->Count--;
		}
		else								//后沿抖动时间已经过去
