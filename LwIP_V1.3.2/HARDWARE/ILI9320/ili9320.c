...
{ 										  
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOG|RCC_APB2Periph_AFIO,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;				 //PA1 推挽输出 背光
	...
	//PORTF0复用推挽输出A0	PF0=A0
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;	 //	//PORTG复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOF, &GPIO_InitStructure); 
	
   	//PORTG12复用推挽输出 	 PG12=NE4
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;	 //	//PORTG复用推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; 		 //复用推挽输出   
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    ...
    readWriteTiming.FSMC_DataSetupTime = 0x30;		 // 数据保存时间为16个HCLK,因为液晶驱动IC的读数据的时候，速度不能太快，尤其对1289这个IC。
    readWriteTiming.FSMC_BusTurnAroundDuration = 0x00;//no use
    readWriteTiming.FSMC_CLKDivision = 0x00;//no use
    readWriteTiming.FSMC_DataLatency = 0x00;//no use
