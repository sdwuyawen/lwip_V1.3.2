...
// 			}
   ...
   //处理返回值，将SPI_SD_driver.c的返回值转成ff.c的返回值
    if(res==0x00)
	{
		disk_read_correct_count++;
		return RES_OK;	 
	}
	else 
	{
		disk_read_error_count++;
		printf("读错误次数%d，读正确次数%d\r\n",disk_read_error_count,disk_read_correct_count);
		return RES_ERROR;	 
	}
}
...
// 			res=0;
    ...
    if(res == 0x00)
	{
		disk_write_correct_count++;
		return RES_OK;	 
	}
	else 
	{
		disk_write_error_count++;
		printf("写错误次数%d，写正确次数%d\r\n",disk_write_error_count,disk_write_correct_count);
		return RES_ERROR;	 
	}	 
}
#endif
