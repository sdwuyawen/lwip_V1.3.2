...
extern FRESULT res;         /* 函数返回值存储变量 */
extern char path0[512];//文件路径
extern char *Create_File_Name;//新创建的文本文件的名字
...
FRESULT scan_files (char* path,uint8_t f_type,uint16_t *latest_num); //搜索文件目录下所有文件 
int SD_TotalSize(char *path);//文件空间占用情况 
uint8_t File_Latest_Name(char *fname,uint16_t *latest_num);//搜索最大文件编号
