...
// receive buffer end
#define RXSTOP_INIT      (8192 - 1514 - 1 - 100)
// start TX buffer at 0x1FFF-0x0600, pace for one full ethernet frame (0~1518 bytes)
#define TXSTART_INIT     (8192 - 1514 - 100)
// stp TX buffer at end of mem
#define TXSTOP_INIT      (8192 - 1 - 100)			/* 发送缓冲区尾部预留100字节给发送状态向量，见ENC28J60手册P40 */

// max frame length which the conroller will accept:
#define   MAX_FRAMELEN    (1518)        // (note: maximum ethernet frame length would be 1518)
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  
#define ENC28J60_CS		PAout(4) 	//神州3号ENC28J60片选信号
//#define ENC28J60_CS		PAout(2) 	//外挂ENC28J60片选信号
// #define ENC28J60_RST	PGout(6)	//ENC28J60复位信号

#define ENC_RX_LED_ON_TIME	20	//LED指示灯亮时间

extern uint8_t ucEIE;	/* ENC28J60寄存器值，用于调试打印 */
extern uint8_t ucEIR;
extern uint8_t ucESTAT;
extern uint8_t ucECON1;
extern uint8_t ucECON2;
extern uint8_t ucEPKTCNT;

extern uint8_t ucERXWRPTL;
extern uint8_t ucERXWRPTH;
extern uint8_t ucERXRDPTL;
extern uint8_t ucERXRDPTH;

extern uint16_t usERXWRPT;
extern uint16_t usERXRDPT;
extern uint32_t ulFreeSpace;
...
void ENC28J60_RX_Reset(void);//执行仅接收复位
uint8_t ENC28J60_Full_Reset(u8* macaddr);//芯片全复位
#endif
